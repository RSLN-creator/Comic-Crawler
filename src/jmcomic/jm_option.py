...
class DirRule:
    ...
    def apply_rule_to_path(self, album, photo, only_album_rules=False) -> str:
        ...
        if not only_album_rules and photo is not None and not has_album_rule:
            episode_count = len(album.episode_list) if hasattr(album, 'episode_list') else 1
            if episode_count > 1:
                album_name = fix_windir_name(JmcomicText.to_zh(album.name, self.normalize_zh)).strip()
                album_dir = f'{album_name}_{album.album_id}'
                if path_ls and path_ls[0] == self.base_dir:
                    path_ls.insert(1, album_dir)
                else:
                    path_ls.insert(0, album_dir)
